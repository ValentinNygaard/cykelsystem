...
public class RepBarLineRepoImpl {
    ...
    public List<RepBarLine> findRepbarsWithStatus(int fromStatus, int toStatus) {
        String sql = "SELECT repair_case_id, customer.phone_number, repair_number, IFNULL(t1.time, 0) AS time, IFNULL(t1.price, 0) AS price, status_title, end_date, customer.name AS customer_name, employee.name AS employee_name, end_time\n" +
                ...
                "order by end_date, end_time, status_id;";
    ...
    public List<RepBarLine> findByStartDate(String fromDate, String toDate) {
        String sql = "SELECT repair_case_id, customer.phone_number, repair_number, IFNULL(t1.time, 0) AS time, IFNULL(t1.price, 0) AS price, status_title, start_date as end_date, customer.name AS customer_name, employee.name AS employee_name, end_time\n" +
                "FROM customer NATURAL JOIN bicycle\n" +
                "NATURAL JOIN repair_case\n" +
                "LEFT JOIN employee ON repair_case.repair_employee_id = employee.employee_id\n" +
                "NATURAL JOIN (SELECT repair_case_id, status_title, sum(time) AS time, sum(price) AS price\n" +
                ...
                "group by repair_case_id\n" +
                "order by start_date, end_time, status_id;";
        RowMapper<RepBarLine> rowMapper = new BeanPropertyRowMapper<>(RepBarLine.class);
